...
namespace learn_Russian_API.Services
{
    public class FileService
    {
        public void SaveFile(List<IFormFile> files, string subDirectory)
        {
            ...
            var folderName = Path.Combine("Resources", subDirectory);
            //var pathToSave = Path.Combine(Directory.GetCurrentDirectory(), folderName);
            var target = folderName; //Path.Combine("D:\\webroot\\", subDirectory);
        ...
        public (string fileType, byte[] archiveData, string archiveName) FetechFiles(string subDirectory)
        {
            ...
            var files = Directory.GetFiles(Path.Combine("Resources", subDirectory)).ToList();
